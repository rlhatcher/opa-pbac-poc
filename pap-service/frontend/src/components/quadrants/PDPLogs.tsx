      ...
      <div className="flex-1 bg-black rounded-md overflow-hidden">
        <ScrollArea className="h-full">
          <div ref={scrollRef} className="p-4 font-mono text-sm space-y-1">
            {logs.length === 0 ? (
              <div className="text-green-400">Waiting for OPA decision logs...</div>
            ) : (
              logs.map((log, index) => (
                <div
                  key={index}
                ...
                >
                  ...
                  <span className={`text-xs font-semibold ${getLogLevelColor(log.level, log.result)}`}>
                    {log.decision || (log.result !== undefined ? (log.result ? 'ALLOW' : 'DENY') : 'LOG')}
                  </span>
